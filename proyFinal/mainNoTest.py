# -------------------------------------------------------------------------
# Archivo completo: ESP32 - control BLE/MQTT/Riego + RGB corregido
# Pegar tal cual en la placa (reemplaza tu archivo actual).
# -------------------------------------------------------------------------
import network
import time
import utime
...
# HARDWARE
# Flotador (ajusta pines si tus sensores usan otros)
...
# Pines RGB (ajusta si usas otros)
l_rojo = PWM(Pin(14), freq=freq)
l_verde = PWM(Pin(27), freq=freq)
l_azul = PWM(Pin(26), freq=freq)

# Si tu LED es ánodo común (common-anode), pon True (inversa la señal)
COMMON_ANODE = False
...
def _parse_combined_colors(s):
    ...
    "medium:rr,gg,bb;empty:rr,gg,bb"
    """
    try:
...
# ------------------- Helpers PWM robustos -------------------
def set_channel_pwm(pwm_obj, value_0_255):
    """
    Establece duty en PWM dado un valor 0..255.
    Soporta duty() (0..1023) o duty_u16() (0..65535).
    Aplica inversión si COMMON_ANODE True.
    """
    try:
        v = int(value_0_255)
        if v < 0: v = 0
    ...
    try:
        ...
        try:
            d65535 = int((d1023 * 65535) / 1023)
            pwm_obj.duty_u16(d65535)
            return True
        except Exception:
            return False
    except Exception:
...
def set_rgb_direct(r, g, b):
    """
    Escribe directamente RGB 0..255 a los PWMs.
    """
    set_channel_pwm(l_rojo, r)
    set_channel_pwm(l_verde, g)
    set_channel_pwm(l_azul, b)

# ------------------- Funciones de estado y LED -------------------
def actualizarEdoContenedor():
    global edoContenedor, sw_Floa_Medium, sw_Floa_Down
    try:
        med = sw_Floa_Medium.value()
        low = sw_Floa_Down.value()
    except Exception:
        med = 0; low = 0
...
def prenderLedRGB(edoContenedor_local):
    """
    Enciende LED RGB según estado y colores configurados.
    """
    global mediumColor, emptyColor
    try:
        if edoContenedor_local == "full":
            # default azul para full (si quieres otro, cambiar aquí)
            set_rgb_direct(0, 0, 255)
        elif edoContenedor_local == "half":
            r,g,b = mediumColor
            set_rgb_direct(r, g, b)
        elif edoContenedor_local == "empty":
            r,g,b = emptyColor
            set_rgb_direct(r, g, b)
        else:
            set_rgb_direct(255,255,255)
    except Exception as e:
        print("prenderLedRGB error:", e)
...
def recibir_info(topic, msg):
    ...
    # plantas
        ...
        # opcional: procesar mensajes de color si los recibes por MQTT
...
# Actualizaciones de estado (temp/hum/...)
...
# decidir_regar (conservé tu implementación completa)
def decidir_regar(
    soil_moisture_pct,
    ambient_temp_c,
...
while True:
    ...
    # Reintentar publishes pendientes (si no hay pulso BLE)
    try:
